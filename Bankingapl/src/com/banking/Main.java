...
public class Main extends JFrame {
    ...
    public Main() {
        ...
        btnDeleteAccount.addActionListener(e -> {
            Deleteaccount ui = new Deleteaccount();
            ui.setVisible(true);
            setVisible(false);
        });
        gbc.gridy = 3;
        ...
        JButton btnSeeAllData = new JButton("See all Data");
        btnSeeAllData.setFont(new Font("Arial", Font.PLAIN, 22));
        btnSeeAllData.setPreferredSize(new Dimension(250, 50));
        btnSeeAllData.setBackground(new Color(0, 102, 204)); // SBI Blue Color
        btnSeeAllData.setForeground(Color.WHITE);
        btnSeeAllData.setFocusPainted(false);
        btnSeeAllData.setCursor(new Cursor(Cursor.HAND_CURSOR));
        btnSeeAllData.setBorder(BorderFactory.createEmptyBorder());
       
        btnSeeAllData.addActionListener(e -> {
        	seealldata sa = new seealldata();
            sa.setVisible(true);
            setVisible(false);
        });
        gbc.gridy = 4;
        centerPanel.add(btnSeeAllData, gbc);
        
        
        JButton btnLoan = new JButton("Loan Details");
        ...
        btnLoan.addActionListener(e -> {
        	AdminLoanView sa = new AdminLoanView();
            sa.setVisible(true);
            setVisible(false);
        });
        gbc.gridy = 5;
